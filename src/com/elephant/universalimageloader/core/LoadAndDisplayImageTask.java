...
import com.elephant.universalimageloader.core.assist.FailReason;
import com.elephant.universalimageloader.core.assist.FailReason.FailType;
import com.elephant.universalimageloader.core.assist.ImageLoadingListener;
import com.elephant.universalimageloader.core.assist.ImageScaleType;
import com.elephant.universalimageloader.core.assist.ImageSize;
import com.elephant.universalimageloader.core.assist.LoadedFrom;
import com.elephant.universalimageloader.core.assist.ViewScaleType;
import com.elephant.universalimageloader.core.decode.ImageDecoder;
import com.elephant.universalimageloader.core.decode.ImageDecodingInfo;
import com.elephant.universalimageloader.core.download.ImageDownloader;
import com.elephant.universalimageloader.core.download.ImageDownloader.Scheme;
import com.elephant.universalimageloader.utils.IoUtils;
...
final class LoadAndDisplayImageTask implements Runnable {
	...
	private Bitmap tryLoadBitmap() {
		...
		try {
			if (imageFile.exists()) {
				...
				bitmap = decodeImage(Scheme.FILE.wrap(imageFile
						.getAbsolutePath()));
				if (imageViewCollected)
					return null;
			}
			if (bitmap == null || bitmap.getWidth() <= 0
				...
				String imageUriForDecoding = options.isCacheOnDisc() ? tryCacheImageOnDisc(imageFile)
						: uri;
				if (!checkTaskIsNotActual()) {
	...
	private Bitmap decodeImage(String imageUri) throws IOException {
		...
		ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey,
				imageUri, targetSize, viewScaleType, getDownloader(), options);
	...
	private boolean downloadSizedImage(File targetFile, int maxWidth,
	...
	private void fireFailEvent(final FailType failType,
				...
				public void run() {
					...
					if (imageView != null) {
						if (options.shouldShowImageResOnFail()) {
							imageView.setImageResource(options
									.getImageResOnFail());
						} else if (options.shouldShowBitmapOnFail()) {
